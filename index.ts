...
export default function (pi: ExtensionAPI) {
  ...
  function saveCache() { cacheManager.saveCache(cache); }
  ...
  function getM(ref: string): Metrics { return metricsModule.getM(ref); }

  function updateMetrics(ref: string, latMs: number, tokens: number, durMs: number) { metricsModule.updateMetrics(ref, latMs, tokens, durMs); }
  ...
  function resolveKeyValue(key: string): string { return discoveryManager.resolveKeyValue(key) ?? key; }

  function loadAuth(): any { return discoveryManager.loadAuth(); }

  function saveAuth(auth: any) { discoveryManager.saveAuth(auth); }

  function isKeyExhausted(prov: string, idx: number): boolean {
    return rateLimitManager.isKeyExhausted(prov, idx);
  }

  function exhaustKey(prov: string, idx: number) {
    rateLimitManager.exhaustKey(prov, idx);
  }

  /** Try rotating to next available key for provider. Returns true if switched. */
  function rotateKey(prov: string): boolean {
    return rateLimitManager.rotateKey(prov, cfg.providers?.[prov]?.keys ?? []);
  }
  ...
  function costMux(prov: string) { return rateLimitManager.costMux(prov); }

  function bumpMux(prov: string, modelId: string) { rateLimitManager.bumpMux(prov, modelId); }

  function isLimited(ref: string) { return rateLimitManager.isLimited(ref); }

  function recordLimit(ref: string): { rotated: boolean; newKey?: string } {
    return rateLimitManager.recordLimit(ref, cfg.providers ?? {});
  }

  function recordOk(ref: string) { rateLimitManager.recordOk(ref); }

  /** Record a soft failure (empty response, timeout) — lighter backoff than 429 */
  function recordSoftFailure(ref: string): void { rateLimitManager.recordSoftFailure(ref); }

  function limitSecs(ref: string) { return rateLimitManager.limitSecs(ref); }

  // ── Usage Stats ────────────────────────────────────────────────────────

function getUsage(ref: string, days: number): number { return metricsModule.getUsage(ref, days); }

function getUsageAll(days: number): Record<string, number> { return metricsModule.getUsageAll(days); }

  // ── Price lookup (OpenRouter as oracle) ─────────────────────────────────

function lookupPrice(ref: string): { input: number; output: number } | null { return metricsModule.lookupPrice(ref); }

  // ── Effective cost ─────────────────────────────────────────────────────

function effCost(ref: string): number { return metricsModule.effCost(ref); }
...
function filterByQualityMin(refs: string[], min: number): string[] { return router.filterByQualityMin(refs, min); }
  ...
  function resolve(name: string): { selected: string; candidates: string[] } | null { return router.resolve(name); }
...
function detectGroup(ref: string): string | null { return router.detectGroup(ref); }
