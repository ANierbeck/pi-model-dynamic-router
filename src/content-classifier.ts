...
export async function classifyPrompt(
  ...
  if (context.lastAssistantSnippet) {
    contextLines.push(`Last assistant response (excerpt): "${context.lastAssistantSnippet.slice(0, 150)}"`);
  }
  const contextBlock = contextLines.length > 0
    ? `Context:\n${contextLines.join("\n")}\n\n`
    : "";

  const ollamaPrompt = CLASSIFICATION_PROMPT
  ...
  try {
    ...
    if (model !== FALLBACK_MODEL) {
      try {
        console.error(`[classifier] Primary model "${model}" failed, retrying with ${FALLBACK_MODEL}:`, (primaryError as Error).message);
